...
public class findUserUsingQueriesTest extends SpringdatajpaApplicationTests {
    ...
    @Test
    void testFindAllByOrderByUsername() {
        List<User> users = userRepository.findAlByOrderByUsernameAsc();
        assertAll(
                () -> assertEquals(10, users.size()),
    ...
    void testFindByUsernameEmail() {
        List<User> userList1= userRepository.findByUsernameAndEmail("mike", "mike@somedomain.com");
        List<User> userList2 = userRepository.findByUsernameOrEmail("mike", "beth@somedomain.com");
        List<User> userList3 = userRepository.findByUsernameAndEmail("mike", "beth@somedomain.com");
        List<User> userList4 = userRepository.findByUsernameOrEmail("beth", "beth@somedomain.com");
    ...
    void testFindByLevelOrderByUsernameDesc() {
        ...
        assertAll(
                ...
                () -> assertEquals("beth", users.get(1).getUsername())
        );
    }
    ...
    void testFindByUserName() {
        List<User> userContaining = userRepository.findByUsernameContaining("ar");
        ...
        assertAll(
                () -> assertEquals(2, userContaining.size()),
    ...
    void testFindByRegistrationDateInNotIn() {
        ...
        LocalDate date2 = LocalDate.of(2020, Month.JANUARY, 5);

        List<LocalDate> dates = new ArrayList<>();
        dates.add(date1);
        dates.add(date2);

        List<User> userList1 = userRepository.findByRegistrationDateIn(dates);
        List<User> userList2 = userRepository.findByRegistrationDateNotIn(dates);

        assertAll(
                () -> assertEquals(3, userList1.size()),
                () -> assertEquals(7, userList2.size())
        );
    }
}
